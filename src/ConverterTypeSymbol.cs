...
{
    [DebuggerDisplay("{ConverterType}")]
    internal class ConverterTypeSymbol : IEquatable<ConverterTypeSymbol>
    {
        ...
        public static ConverterTypeSymbol? GetConverter(SourceProductionContext context, TypeSymbol propertySymbol)
        {
            if (propertySymbol.Symbol == null)
                return null;

            var attributes = propertySymbol.Symbol.GetAttributes();
            var converterAttr = attributes.FirstOrDefault(c => c.AttributeClass?.Name.Equals(nameof(CacheConverterAttribute), StringComparison.Ordinal) == true);
            ...
            while (baseType != null)
            {
                if (baseType.Name.Equals(nameof(CacheValueConverter), StringComparison.Ordinal))
                {
                    if (baseType.TypeArguments.Length != 1)
                    {
                        context.ReportDiagnostic(Diagnostic.Create(SourceGeneratorDiagnostics.MismatchedConverterGenericType, propertySymbol.Symbol.Locations.FirstOrDefault(), propertySymbol.Symbol.Name));
                        return null;
                    }
            ...
            if (baseType == null || !baseType.Name.Equals(nameof(CacheValueConverter), StringComparison.Ordinal))
                return null;

            var targetTypeSymbol = TypeSymbol.Create(context, baseType.TypeArguments[0]);
